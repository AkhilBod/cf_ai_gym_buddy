...
var GymBuddyAgent = class {
  static {
    __name(this, "GymBuddyAgent");
  }
  state;
  env;
...
var src_default = {
  async fetch(req, env2) {
    ...
    if (url.pathname === "/api/start" && req.method === "POST") {
      ...
      const id = env2.AGENT.idFromName(sessionId);
      const stub = env2.AGENT.get(id);
    ...
    if (url.pathname === "/api/start-workflow" && req.method === "POST") {
      ...
      const run = await env2.INTERVIEW.create({ params: { role } });
...
// .wrangler/tmp/bundle-JTnx7f/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-JTnx7f/middleware-loader.entry.ts
